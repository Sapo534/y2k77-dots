...
check_no_nested_config() {
  # ловит кривую структуру вида pkg/.config/pkg/.config/... до того,
  # как она разлетится симлинками по всему $HOME
  local pkg="$1"
  local bad
  bad=$(find "$DOTFILES_DIR/$pkg" -mindepth 2 -type d -name '.config' 2>/dev/null || true)
  if [[ -n "$bad" ]]; then
    echo "!! Пакет '$pkg' содержит вложенную .config внутри себя — похоже на повреждённую структуру:" >&2
    echo "$bad" >&2
    echo "!! Останавливаюсь, чтобы не разложить мусор по \$HOME. Почини вручную и перезапусти." >&2
    exit 1
  fi
}
